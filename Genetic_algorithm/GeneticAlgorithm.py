...
class GeneticAlgorithmApp:
    def __init__(self, root):
        ...
        self.encodingType = tk.BooleanVar(value=False)
        ...
        self.useSelectionModification = tk.BooleanVar(value=True)
        tk.Checkbutton(root, text="Использовать модификацию выбора родителя", variable=self.useSelectionModification).grid(row=7, column=0, columnspan=2, sticky="w")

        tk.Button(root, text="Рассчитать хромосомы", command=self.runAlgorithm).grid(row=8, column=0, columnspan=2, sticky="we")

        tk.Label(root, text="Количество прошлых поколений: ").grid(row=9, column=0, sticky="w")
        self.pastGenerations = tk.Label(root, text="0")
        self.pastGenerations.grid(row=10, column=1, sticky="w")

        tk.Label(root, text="Лучшие решения:").grid(row=10, column=0, columnspan=2, sticky="w")
        self.resultX1 = tk.Label(root, text="x1 = ")
        self.resultX1.grid(row=11, column=0, sticky="w")
        self.resultX2 = tk.Label(root, text="x2 = ")
        self.resultX2.grid(row=12, column=0, sticky="w")
        self.resultFitness = tk.Label(root, text="Значение функции = ")
        self.resultFitness.grid(row=13, column=0, columnspan=2, sticky="w")

        self.historyTable = ttk.Treeview(root, columns=("Index", "x1", "x2", "Fitness"), show='headings')
        self.historyTable.grid(row=0, column=3, rowspan=14, padx=5, sticky="nswe")
    ...
    def initializePopulation(self, numChromosomes, minGene, maxGene):
        if self.encodingType.get():
            return np.random.randint(int(minGene), int(maxGene) + 1, (numChromosomes, 2))
        else:
    ...
    def tournamentSelection(self, population, fitness, k=7, probablyTheBest=0.9):
        ...
        for _ in range(2):
            ...
            if self.useSelectionModification.get():
